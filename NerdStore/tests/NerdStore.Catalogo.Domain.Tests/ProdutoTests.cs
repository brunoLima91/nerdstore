...
namespace NerdStore.Catalogo.Domain.Tests
{
	public class ProdutoTests
	{
		...
		public void Produto_Validar_ValidacoesDevemRetornarExceptions()
		{
			...
			ex = Assert.Throws<DomainException>(() =>
				new Produto("Nome", "Descricao", false, 10, Guid.Empty, DateTime.Now, "imagem", new Dimensoes(1, 1, 1))
			);
			...
			ex = Assert.Throws<DomainException>(() =>
				new Produto("Nome", "Descricao", false, 10, Guid.NewGuid(), DateTime.Now, string.Empty, new Dimensoes(1, 1, 1))
			);

			Assert.Equal("O campo Imagem do produto não pode estar vazio", ex.Message);

			ex = Assert.Throws<DomainException>(() =>
				new Produto("Nome", "Descricao", false, 10, Guid.NewGuid(), DateTime.Now, "imagem", new Dimensoes(0, 1, 1))
			);

			Assert.Equal("O campo Altura não pode ser menor ou igual a 0", ex.Message);

			ex = Assert.Throws<DomainException>(() =>
				new Produto("Nome", "Descricao", false, 10, Guid.NewGuid(), DateTime.Now, "imagem", new Dimensoes(1, 0, 1))
			);
			...
			ex = Assert.Throws<DomainException>(() =>
				new Produto("Nome", "Descricao", false, 10, Guid.NewGuid(), DateTime.Now, "imagem", new Dimensoes(1, 1, 0))
			);
